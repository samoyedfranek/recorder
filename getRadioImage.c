...
static int read_frame()
{
    ...
    while (1)
    {
        ...
        if (b == HEADER0)
        {
            ...
            if (read(serial_fd, &t, 1) <= 0)
                return 0;
            if (read(serial_fd, size_bytes, 2) <= 0)
                return 0;

            int size = (size_bytes[0] << 8) | size_bytes[1];
            if (t == TYPE_SCREENSHOT && size == FRAME_SIZE)
            {
                if (read(serial_fd, framebuffer, FRAME_SIZE) <= 0)
                    return 0;
                return 1;
            }
...
{
    if (memcmp(framebuffer, last_frame, FRAME_SIZE) == 0)
        return;
