    ...
    try:
        while True:
            ...
            if debug:
                ...
                print(f"Audio chunk size: {len(indata)}")
                print(f"Silence check: {time.time() - last_sound_time if last_sound_time else 'N/A'} seconds since last sound")
            if max_amplitude > AMPLITUDE_THRESHOLD:
                ...
                if last_sound_time is not None:
                    print(f"Silent chunk detected: {time.time() - last_sound_time:.2f} seconds of silence")
                last_sound_time = time.time()
